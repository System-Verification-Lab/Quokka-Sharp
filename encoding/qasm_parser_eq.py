import random
class Circuit:
    def __init__(self, n, translate_ccx):
        self.n = n
        self.tgate = 0
        self.cxgate = 0
        self.circ = []
...
def ADD(circuit1, circuit2, n):
        circuit3 = Circuit(n, False)
...
def qasm_parser(filename, n, translate_ccx, reverse):
    qasm_list = []
    ...
    circuit = Circuit(n,translate_ccx)
...
def eq_parser_gm(file1, SEED):
    circuit1 = qasm_parser(file1, 0, False, False)
    circuit2 = Circuit(circuit2.n, False)
    random.seed(SEED)
    idx = random.randint(0, len(circuit1.circ) - 1)
    missgate = circuit1.circ[idx]
    if missgate[0] == 't':
        circuit2.tgate = circuit1.tgate - 1
    elif missgate[0] == 'cx':
        circuit2.cxgate = circuit1.cxgate - 1
    del circuit2.circ[idx]
    
    circuit3 = ADD(circuit1, circuit2, circuit1.n * 2)
    return circuit3
