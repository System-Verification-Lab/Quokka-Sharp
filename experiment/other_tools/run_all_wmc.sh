...
date "+%Y-%m-%d %H:%M:%S"
echo 'running eq check in pauli basis'
./run_eq_wmc.sh $1 pauli $2 > results/eq_wmc_pauli_results.txt

date "+%Y-%m-%d %H:%M:%S"
echo 'running eq check in comp basis'
./run_eq_wmc.sh $1 comp  $2 > results/eq_wmc_comp_results.txt

# get eq test results for a sample of the benchmarks

date "+%Y-%m-%d %H:%M:%S"
echo 'running sim check with allzero mesurment in pauli basis'
./run_sim_wmc.sh $1 allzero pauli   $2 > results/sim_wmc_allzero_pauli_results.txt

date "+%Y-%m-%d %H:%M:%S"
echo 'running sim check with allzero mesurment in comp basis'
