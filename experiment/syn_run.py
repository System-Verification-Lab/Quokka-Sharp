...
def main(tool_path, qasmfile, eq_tool_path=None):
    ...
    for comp_basis, onehot, sandwich in [(False, True, False)]: #,(True, False, False), (False, False, False)]: #(True, False, False), (False, False, False), , (False, False, True), (False, True, True)
        ...
        res, weight, sol, layers = qk.Synthesis(tool_path, cnf, cnf_file_root=helper_folder, fidelity_threshold=1, bin_search=False, initial_depth=0, onehot_xz = onehot, h_sandwich=sandwich)
        ...
        print(f"\n *** *** ***\t Time: {runtime:.2f}\tResult: {res}\tbest weight: {weight} \t", end="")
        ...
        with open(helper_folder+"final_results.txt", "a") as file:
            file.write(f"{("comp" if comp_basis else "pauli")}\t{("2n" if onehot else "full")}\t{res}\t{layers}\t{runtime}\t{weight:.5f}\t{sol_depth}\t{org_depth}\n")
