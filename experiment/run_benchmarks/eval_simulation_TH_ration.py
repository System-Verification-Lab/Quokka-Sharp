...
df_columns = ["qubits", "deepth", "seed", "ratio", "measurement", "basis", "result", "time"]
...
def draw_figures(results_df, results_file_name):
	...
	for exp, results in results_df.groupby(["qubits", "deepth", "seed", "ratio", "measurement"]):
	...
	line_map = {d: next(lines) for d in sorted(results_df["deepth"].unique())}
	...
	for qubits in results_df["qubits"].unique():
		...
		for (deepth, basis), group in qubits_df.groupby(["deepth", "basis"]):
			...
			plt.errorbar(
				...
				group["time"]["mean"],
				yerr=group["time"]["std"],
				label=f"deepth={deepth}, {basis}",
				fmt='o',
				capsize=5,
				color=color_map[basis],
				linestyle=line_map[deepth]
			)
...
for file in tqdm(benchmarks_list, desc="Processing files", unit="file"):
	...
	qubits, deepth, seed, ratio = get_from_file_name(file)
...
results_df.sort_values(by=["qubits", "deepth", "seed", "ratio", "measurement", "basis"], inplace=True)
