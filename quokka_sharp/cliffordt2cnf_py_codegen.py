...
def add_sign(func, prefix = ""):
    w_R_func = Equivalent(R, func)
    w_R_cnf = to_cnf(w_R_func, True, True)
    c_R_func = Equivalent(R, r ^ func)
    c_R_cnf = to_cnf(c_R_func, True, True)
    print(prefix+"        R = cnf.add_var()")
    print(prefix+"        if cnf.weighted: ")
    print(prefix+"            cnf.add_weight(R, -1)")
    print(prefix+"            cnf.add_weight(-R, 1)")
    print(prefix+"            # "+ str(w_R_func))
    to_py(	                  str(w_R_cnf), prefix+"    ")
    print(prefix+"        else: ")
    print(prefix+"            r = cnf.vars.r")
    print(prefix+"            cnf.vars.r = R")
    print(prefix+"            # "+ str(c_R_func))
    to_py(	                  str(c_R_cnf), prefix+"    ")

def add_sqrt_half(func, prefix = ""):
    w_U_func = Equivalent(U, func)
    w_U_cnf = to_cnf(w_U_func, True, True)
    c_U_func = Equivalent(U, u ^ ~func)
    c_U_cnf = to_cnf(c_U_func, True, True)
    c_D_func = Equivalent(D, u & ~U)
    c_D_cnf = to_cnf(c_D_func, True, True)
    print(prefix+"        U = cnf.add_var()")
    print(prefix+"        if cnf.weighted: ")
    print(prefix+"            cnf.add_weight(U, str(Decimal(1/2).sqrt()))")
    print(prefix+"            cnf.add_weight(-U, 1)")
    print(prefix+"            # "+ str(w_U_func))
    to_py(	                  str(w_U_cnf), prefix+"    ")
    print(prefix+"        else: ")
    print(prefix+"            cnf.power_two_normalisation += 0.5 ")
    print(prefix+"            u = cnf.vars.u")
    print(prefix+"            cnf.vars.u = U")
    print(prefix+"            # "+ str(c_U_func))
    to_py(	                  str(c_U_cnf), prefix+"    ")
    print()
    print(prefix+"            D = cnf.add_var()")
    print(prefix+"            cnf.add_clause([D, cnf.add_var()])")
    print(prefix+"            # "+ str(c_D_func))
    to_py(	                  str(c_D_cnf), prefix+"    ")
